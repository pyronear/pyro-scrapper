...
# Import Engine from pyroengine.core as documented in README
...
def run_inference_on_sequence(engine: Engine, sequence: List[ImageEntry], conf_thresh: float = 0.15) -> Tuple[bool, float]:
...
def main(images_dir: Optional[str], n: int, max_gap_seconds: int, conf_thresh: float, output_dir: Optional[str]) -> int:
	...
	print(f"Initializing pyroengine...")
	try:
		engine = Engine()
	...
	for folder in iter_leaf_folders(root):
		...
		for idx, seq in enumerate(seqs, 1):
			...
			print(f"  Sequence #{idx}: {first_ts} -> {last_ts} ({len(seq)} images)", end=" ")
...
if __name__ == "__main__":
	...
	parser.add_argument("--images-dir", type=str, default=None, help="Root images directory (defaults to images/ next to this file)")
	parser.add_argument("--n", type=int, default=6, help="Required number of consecutive images")
	parser.add_argument("--max-gap", type=int, default=60, help="Maximum allowed gap in seconds between consecutive images")
	parser.add_argument("--conf-thresh", type=float, default=0.15, help="Confidence threshold for detection")
	parser.add_argument("--output-dir", type=str, default=None, help="Output directory for detected sequences (defaults to annotations/ next to images)")
